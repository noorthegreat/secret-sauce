    ...
    <div className="h-full overflow-y-auto pb-28">
      <div className="pt-3">
        ...
        <p className="mt-2 px-6 text-sm leading-relaxed text-muted-foreground">
          A short, considered list — not a directory. Introductions open a meetup only once you both accept.
        </p>
      </div>

      <div className="mt-6 flex flex-col gap-4 px-6">
        {residents.slice(1).map((resident) => {
          const status = statuses[resident.id] ?? "idle"

          return (
            <article key={resident.id} className="overflow-hidden rounded-3xl border border-border bg-card">
              <div className="flex gap-4 p-4">
                ...
                <div className="min-w-0 flex-1">
                  <p className="text-[11px] text-muted-foreground">{resident.unit}</p>
                  <h3 className="font-serif text-xl leading-tight text-foreground">{resident.name}</h3>
                  <span className="mt-1 inline-flex items-center gap-1 rounded-full bg-gold/15 px-2.5 py-0.5 text-[11px] text-gold-foreground">
                    <Sparkles className="size-3 text-gold" /> {resident.goal}
                  </span>
                </div>
              </div>
              <p className="px-4 text-sm leading-relaxed text-foreground/75">{resident.tagline}</p>
              ...
              <div className="p-4">
                ...
                {status === "requested" && (
                  <div className="flex w-full items-center justify-center gap-2 rounded-full border border-border bg-secondary py-3.5 text-sm font-medium text-muted-foreground">
                    ...
                    Awaiting {resident.name.split(" ")[0]}&apos;s response
                  </div>
                )}
                {status === "matched" && (
