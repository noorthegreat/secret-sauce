...
export const SWISS_CITY_OPTIONS = SWISS_CITY_INSTITUTION_GROUPS.map(({ city }) => city);

export const SWISS_INSTITUTIONS_BY_CITY = Object.fromEntries(
  SWISS_CITY_INSTITUTION_GROUPS.map(({ city, institutions }) => [city, institutions]),
) as Record<string, SwissInstitution[]>;
